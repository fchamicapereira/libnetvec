...
void test_puts(const unsigned capacity, const unsigned key_size, const unsigned total_puts) {
  MapVec8 map1(capacity, key_size);
  MapVec8 map2(capacity, key_size);
  ...
  while (map2.get_size() < total_puts) {
    u8 keys[MapVec8::VECTOR_SIZE * key_size];
    int values[MapVec8::VECTOR_SIZE];

    for (int i = 0; i < MapVec8::VECTOR_SIZE; i++) {
...
void test_gets(const unsigned capacity, const unsigned key_size, const unsigned total_gets) {
  ...
  while (map.get_size() < total_gets) {
    ...
    int new_values[MapVec8::VECTOR_SIZE];
    for (int i = 0; i < MapVec8::VECTOR_SIZE; i++) {
      new_values[i] = 0;
    }
...
int main() {
  test_puts(65536, 16, 16);
  test_puts(32, 16, 32);
  test_puts(65536, 16, 65536);
  test_gets(65536, 16, 16);
  test_gets(65536, 16, 32);
  test_gets(65536, 16, 65536);
  test_unsuccessful_gets(65536, 16, 16);
  return 0;
}
...
inline int find_key(int *busybits, void **keyps, unsigned *k_hashes, void *keyp, unsigned key_size, unsigned key_hash, unsigned capacity) {
...
inline unsigned find_empty(int *busybits, unsigned start, unsigned capacity) {
  for (unsigned i = 0; i < capacity; ++i) {
    unsigned index = loop(start + i, capacity);
    int bb         = busybits[index];
    if (0 == bb) {
...
inline unsigned find_key_remove_chain(int *busybits, void **keyps, unsigned *k_hashes, void *keyp, unsigned key_size, unsigned key_hash, unsigned capacity) {
  unsigned i     = 0;
  unsigned start = loop(key_hash, capacity);
